...
bool Planner::moveConstantVelocity2(float rpm)
{
  //float finalAngle = 100000.0;
  bool state;
  state = enterTC3CriticalSection();
  ...
  //first determine if operation is in progress
  if (PLANNER_NONE != currentMode && rpm == 0)
  {
    //we are in operation return false
    SerialUSB.println("planner operational");
    runContinous = false;
    endAngle = currentSetAngle;
    exitTC3CriticalSection(state);
    return false;
  }
...
//    SerialUSB.println();
  ...
  runContinous = true;  
  SerialUSB.print("runContinous ");
  SerialUSB.println(runContinous);
